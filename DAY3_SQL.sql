...
purchase_cost decimal(6,2), Estimated_sale_price float4, Month int);
...
Select month, product, count(*) from
Bank_Inventory_Pricing where Month=1 group by Product having count(*) >1;
...
select Product, count(Product) from bank_inventory_pricing where purchase_cost>ifnull(Estimated_sale_price,0)
...
select avg(coalesce(purchase_cost, Estimated_sale_price)) as new_avg from bank_inventory_pricing;
# Question 8:
...
select product, (max(ifnull(purchase_cost, 0)) - min(ifnull(purchase_cost, 0))) 
as dif_new from Bank_Inventory_pricing group by product;
# Question 10:
# 10) Print the sum of Purchase_cost of Bank_inventory_pricing during 1st and 2nd month
select Month, sum(coalesce(purchase_cost, 0)) as cash from Bank_Inventory_pricing
group by Month having Month= 1 or Month=2;
...
sum(coalesce(purchase_cost, 0)) as new_cost from Bank_Inventory_pricing group by product 
...
select product, avg(coalesce(purchase_cost, Estimated_sale_price)) as new_sale from Bank_Inventory_pricing 
group by product;
...
select product, avg(coalesce(purchase_cost, 0)) as new_avg from Bank_Inventory_pricing group by product 
...
select product,
case
	when purchase_cost is null then round(avg(if(price>Estimated_sale_price, price, Estimated_sale_price)),2)
...
# For any null estimated_sale_price, replace the value with purchase_cost
 select Product,round(avg(ifnull(purchase_cost,greatest(Price,Estimated_sale_price))) ,2)
 from Bank_Inventory_pricing where Product='BusiCard';
